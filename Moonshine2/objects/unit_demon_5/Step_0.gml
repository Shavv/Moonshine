...
if spritestate="attack"
{
 ...
 walk=false
 look_to_target()
 if alarm[0]=-1 or image_ignore=asset_get_index(sprite_attack)  {set_attack_ranged_stop(asset_get_index(sprite_attack),0.2,10,f_sta*0.3,unit_demon_5_breath,x+image_xscale*5,y-10)}
 ...
 if image_index>4
 {
  if frame>20
  {
   if instance_exists(target) {dmg_done(target,f_sta/15)}
   frame=0
  }
   else
  {
