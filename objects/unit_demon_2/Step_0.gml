...
{ 
 ...
 walk=false
 look_to_target()
 if alarm[0]=-1 or image_ignore=asset_get_index(sprite_attack)
 {
  set_attack_melee(asset_get_index(sprite_attack),0.2,6,f_str)
  if image_index=11  {attacked=false}
  if image_index>=13 {set_attack_melee(asset_get_index(sprite_attack),0.2,15,f_str)}
 }
}   
